            ...
            if original.activa != self.activa:
                ...
                if not self.activa:
                    if hasattr(self.maquina_vulnerable, 'maquinadocker'):
                        ...
                        comando=f"docker stop {nombre_maquina}_{self.jugador.usuario.username}"
                        subprocess.run(comando, shell=True, check=True)
                        comando=f"docker rm {nombre_maquina}_{self.jugador.usuario.username}"
                        subprocess.run(comando, shell=True, check=True)
                        if self.ip_address:
                    ...
                    if hasattr(self.maquina_vulnerable, 'maquinadocker'):
                        ...
                        comando = f"cd maquinas_docker/{nombre_maquina}/ && docker build -t apache-default default_conf"
                        subprocess.run(comando, shell=True, check=True)
                        comando = f"docker run -d --name {nombre_maquina}_{self.jugador.usuario.username} apache-default"
                        subprocess.run(comando, shell=True, check=True)

                        # Obtengo la ip
                        comando = f"docker inspect -f '{{{{range .NetworkSettings.Networks}}}}{{{{.IPAddress}}}}{{{{end}}}}' {nombre_maquina}_{self.jugador.usuario.username}"
